...
def build_reactions_from_genbank(me_model, gb_filename, tu_frame=None,
    ...
    for feature in gb_file.features:
        ...
        if feature.type == 'CDS' or forced:
            ...
            rna_type = 'ncRNA'
            warn('Processing '+bnum+' as ncRNA')
        elif feature.type == 'gene':
            warn('Processing '+bnum+' as mRNA')
            rna_type = 'mRNA'
        else:
        ...
        if len(seq) % 3 != 0:
            if frameshift_string:
                warn('Applying frameshift on %s' % bnum)
                seq = dogma.return_frameshift_sequence(full_seq, frameshift_string)
                if strand == '-':
                    seq = dogma.reverse_transcribe(seq)
            elif bnum in force_include_genes:
                n = len(seq) % 3
                seq = seq[:-n]
                warn('Ignoring last {} nucleotides on {}'.format(n,bnum))
