...
class TranscriptionReaction(MEReaction):
    ...
    def __init__(self, id):
        ...
        self.krnap_penalty = 1. # Added to simulate inhibition (JDTB, Dec 2021)
    ...
    def update(self, verbose=True):
        ...
        try:
            ...
            warn("RNA Polymerase {} not found".format(rna_polymerase))
        else:
            if not hasattr(self,'krnap_penalty'):
                self.krnap_penalty = 0.
            k_rnap = (mu * c_ribo * kt / (mu + kt * r0)) * 3 * (self.krnap_penalty)  # (3*k_ribo) hr-1
...
class TranslationReaction(MEReaction):
    ...
    def update(self, verbose=True):
        ...
        self.add_metabolites(object_stoichiometry, combine=False)
#         print(5,protein_id,metabolites.get_by_id(protein_id).formula_weight,translation_data.amino_acid_sequence)
        # -------------Update Element Dictionary and Formula-------------------
        self._add_formula_to_protein(translation_data, protein)
#         print(6,protein_id,metabolites.get_by_id(protein_id).formula_weight,translation_data.amino_acid_sequence)
